    ...
    for x in pdf_files:
        try:
            print(Fore.RED + "URL: " + Fore.CYAN + x + Fore.RED + " Size: " + Fore.LIGHTRED_EX +
                  requests.get(x, stream=True).headers['Content-length'] + " Bytes")
            size = size + int(requests.get(x, stream=True).headers['Content-length'])
            size_table.add_row(
                [x + Fore.RED, str(round(int(requests.get(x, stream=True).headers['Content-length']) / 1048576,
                                         3)) + " MB" + Fore.LIGHTRED_EX])
        except():
            print("ERROR")

    print("")
    print(Fore.LIGHTGREEN_EX + "*" * 200)
    print(size_table.get_string(sort_key=operator.itemgetter(1, 0), sortby="File Size"))
    return size
...
scrape(url_main)
scrape_file()
file_size = get_download_size()
file_size = file_size / 1048576
print("")
print("")
print("Total Size: ~" + str(round(file_size)) + " MB")
print("")
print("")
print("*" * 200)
print("")
